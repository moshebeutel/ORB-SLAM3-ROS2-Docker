...
slam_msgs__srv__GetGlobalPointCloud_Request__are_equal(const slam_msgs__srv__GetGlobalPointCloud_Request * lhs, const slam_msgs__srv__GetGlobalPointCloud_Request * rhs)
{
  ...
  // map_pitch
  if (lhs->map_pitch != rhs->map_pitch) {
    return false;
  }
  // map_roll
  if (lhs->map_roll != rhs->map_roll) {
    return false;
  }
  // map_yaw
  if (lhs->map_yaw != rhs->map_yaw) {
    return false;
  }
  // global_voxel_resolution
  ...
  // ground_color_red
  if (lhs->ground_color_red != rhs->ground_color_red) {
    return false;
  }
  // ground_color_green
  if (lhs->ground_color_green != rhs->ground_color_green) {
    return false;
  }
  // ground_color_blue
  if (lhs->ground_color_blue != rhs->ground_color_blue) {
    return false;
  }
  // z_thresh_min
  if (lhs->z_thresh_min != rhs->z_thresh_min) {
    return false;
  }
  // z_thresh_max
  ...
  slam_msgs__srv__GetGlobalPointCloud_Request * output)
{
  ...
  // map_pitch
  output->map_pitch = input->map_pitch;
  // map_roll
  output->map_roll = input->map_roll;
  // map_yaw
  output->map_yaw = input->map_yaw;
  // global_voxel_resolution
  output->global_voxel_resolution = input->global_voxel_resolution;
  // local_voxel_resolution
  output->local_voxel_resolution = input->local_voxel_resolution;
  // ground_color_red
  output->ground_color_red = input->ground_color_red;
  // ground_color_green
  output->ground_color_green = input->ground_color_green;
  // ground_color_blue
  output->ground_color_blue = input->ground_color_blue;
  // z_thresh_min
  output->z_thresh_min = input->z_thresh_min;
  // z_thresh_max
